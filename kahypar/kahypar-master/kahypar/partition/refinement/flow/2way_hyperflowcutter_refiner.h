 ...
 private:
  ...
  bool refineImpl(std::vector<HypernodeID>&, const std::array<HypernodeWeight, 2>&,
    ...
    while (should_continue) {
      ...
      auto STF = extractor.run(_hg, _context, cut_hes, b0, b1, hfc.cs.border_nodes.distance);
      ...
      hfc.reset();
      hfc.setFlowBound = STF.cutAtStake - STF.baseCut;
      bool flowcutter_succeeded = hfc.enumerateCutsUntilBalancedOrFlowBoundExceeded(STF.source, STF.target);
      HyperedgeWeight newCut = STF.baseCut + hfc.cs.flow_algo;
